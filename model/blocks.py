...
class ResBlock(TimestepBlock):
    ...
    def forward(self, x, emb=None, cond=None, lateral=None):
        ...
        """
        return torch_checkpoint(self._forward, (x, emb, None, lateral),
...
class AttentionBlock(nn.Module):
    ...
    def _forward(self, x):
        ...
        _sz = int(math.sqrt(qkv.shape[-1] // self.z_size))
        n_h, n_w = _sz // 8, _sz // 8
        if self.is_half and (n_h > 1 or n_w > 1):
            qkv = rearrange(qkv, 
                            'b c (z h w) -> b c z h w',
                            h=_sz, w=_sz)
            qkv = rearrange(qkv, 
                            'b c z (n_h h) (n_w w) -> (b n_h n_w) c (z h w)',
                            n_h=n_h, n_w=n_w)
        ...
        h = self.proj_out(h)
        if self.is_half and (n_h > 1 or n_w > 1):
            h = rearrange(h, '(b n_h n_w) c (z h w) -> b c z (n_h h) (n_w w)',
                          n_h=n_h, n_w=n_w, h=8, w=8)
            h = rearrange(h, 'b c z h w -> b c (z h w)')
